...
#     input_path = Path(__file__).parent/"input"/"PDF_test"/"20210930_REGOLAMENTO_SVT_2021.pdf"
#     output_path = Path(__file__).parent/"input"/"PDF_test"/"20210930_REGOLAMENTO_SVT_2021.json"
...
file_json= load_json(Path(__file__).parent/"input"/"PDF_test"/"20210930_REGOLAMENTO_SVT_2021.json")

# Supponendo che `result` sia il dizionario restituito dal parser
sezioni_raw = file_json["sezioni"]

sezioni = json.loads(sezioni_raw)

# Ora puoi iterare normalmente
...
for sezione in sezioni:
    titolo = sezione.get("titolo", "")
    contenuto = sezione.get("contenuto", "")
    if "articolo 6" in titolo.lower():  # match case-insensitive
        print(f"\n📘 {titolo}\n{'-' * 80}\n{contenuto}\n{'=' * 80}")
        break
else:
    print(" Articolo 6 non trovato nel documento.")